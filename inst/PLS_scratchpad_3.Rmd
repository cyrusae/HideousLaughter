...
get_pls_data <- \(url, extract = 'fy20..', 
  ...
  zip_contents <- grep('\\w+\\.csv$', #get paths to the unzipped CSVs
                       list.files(fp, full.names = TRUE),
                       ignore.case = TRUE, value = TRUE)
  writeLines(paste0('Zip contents: ', zip_contents))
...
```
TODO: Test the above (currently has not run but is written); move to `pls.R`. 04/02 breakpoint (dinner).
```{r}
test <- get_pls_data(url = pls['fy2017'])
```
```{r}
purrr::map(.x = pls,
                  .f = get_pls_data)
